...
`crypto.py` — `fetch_crypto_ohlcv(instrument, market, limit, frequency)`: a free, **keyless** crypto fetcher (CCData spot API, paginated backwards over the 100‑rows/call limit via `to_ts`). Returns the standard OHLCV contract **plus `vbuy`/`vsell`** — a single venue's **real aggressor‑side buy/sell volume** — enabling experiments on *real* order flow (vs the synthetic close‑location‑value proxy) on survivorship‑light majors. Parse logic is pure/offline‑testable; the live HTTP call lives only in the crypto examples — **experiment 14** (`crypto_realvol.py`, daily flow vs the synthetic proxy) and **experiment 15** (`crypto_intraday_profile.py`, the real *intraday* profile via `frequency="hours"`).
