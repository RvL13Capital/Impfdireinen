...
python -m pytest -q                                    # 158 offline, deterministic tests
```
...
├─ data/                   OHLCV fetchers — yfinance (equity) + keyless crypto (real buy/sell volume)
...
```bash
python -m pytest -q            # 158 tests, all offline & deterministic (no network)
